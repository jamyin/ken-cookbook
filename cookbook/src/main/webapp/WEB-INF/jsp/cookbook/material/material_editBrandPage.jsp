...
<script type="text/javascript">
	...
	function hasBrandName(name){
		$.ajax({
			type: "POST",
			url: '<%=basePath%>brandController/hasBrandName.do?brandName='+name,
			dataType:'json',
			success: function(data){
				if(data.length>0){
					bootbox.alert("该品牌名称已经存在");
					return false;
				}else{
					$("#materialbrandForm").submit();
					$("#zhongxin").hide();
					$("#zhongxin2").show();
				}
			}
...
<body>
	<form action="materialController/updateBrand.do" name="materialbrandForm" id="materialbrandForm" method="post">
		...
		<table>
		    ...
		    <tr>
				...
				<td><input type="text" name="name" id="name"  maxlength="32" placeholder="这里输入品牌名称" title="名称" value="${brandDto.name}" /></td>
			</tr>
			<tr>
