	...
	function save(){
		...
		if($("#cost").val()==""){
			...
			return false;
			 ...
			 if(!validate(cost_val)){
				  bootbox.alert("原料成本只能含有数字和.");
				  return false;
			 }else if(cost_val.length>6){
		...
		if($("#remark").val()==""){
		   ...
		   if($(".select_wrap")[0].style.visibility=='hidden'){
			   ...
			   //   console.log($($($($($(sn[j])[0]).children()[1])[0])[0]).context);
		...
		/* 		 console.log(  $($($($(sn[j])[0]).children()[1])[0]).val());
				 console.log(  $($($($(sn[j])[0]).children()[2])[0]).val()); */
			   }
...
<body>
	<form action="materialController/saveMaterial.do" name="materialForm" id="materialForm" method="post">
	   ...
	   <input id="imgUrl" name="imgUrl" type="hidden" value="/upload/aaa" />
		...
		<table id="mlist_page">
		    <tr>
				...
				<td><input type="text" name="name" id="name"  maxlength="32" placeholder="这里输入原料名称" title="名称"/></td>
			</tr>
			<tr >
			    <td style="text-align:left">大类：</td>
				<td style="position: relative;padding-bottom:11px;">
				...
				<c:forEach items="${listMB}" var="bigcate">
					<option value="${bigcate.id }" >${bigcate.name }</option>
				</c:forEach>
				</select>
				<i id="bigCategoryIdiconplus" class="icon icon-plus input_big_plus"></i>
				</td>
			...
			<tr >
				...
				<i id="brandIdiconplus" class="icon icon-plus input_brand_plus"></i>
				</td>
			</tr>
		    <tr>
		        <td style="text-align:left；padding-top:7px;">类型：</td>
		        <td style="padding-top:7px;">
				   <label style="float:left;padding-left: 20px;"><input name="type" id="type"  type="radio" value="1" checked="checked"><span class="lbl" style="width:60px">原料</span></label>
		           <label style="float:left;padding-left: 20px;"><input name="type" id="type"  type="radio" value="2" ><span class="lbl" style="width:60px">半成品</span></label>
		        </td>
		    </tr>
		     <tr>
		        <td style="text-align:left;padding-top:7px;">敏感食材：</td>
		        <td style="padding-top:7px;">
				   <label style="float:left;padding-left: 20px;"><input name="isSensitiveMaterial" id="isSensitiveMaterial"  type="radio" value="0" checked="checked"><span class="lbl" style="width:60px">否</span></label>
		           <label style="float:left;padding-left: 20px;"><input name="isSensitiveMaterial" id="isSensitiveMaterial"  type="radio" value="1" ><span class="lbl" style="width:60px">是</span></label>
		        </td>
		    </tr>
			<tr>
		        <td style="text-align:left">成本：</td>
				<td style="padding-top:13px;"><input type="text" name="cost" id="cost"  maxlength="32" style="width: 90px;" placeholder="这里输入成本" title="成本"/><span style="top:-3px;position: relative;">/元</span></td>
			</tr>
			<tr >
			 <td style="vertical-align: top;">营养:</td>
			 <td>
			    <div id="selectNutr" class="dwrap" style="width:276px;">
			       <div  class="select_wrap" style="width: 214px; margin-left:0;visibility: hidden;">
			       <input  type="text" class="kel" name="nutrname" value="" style="width: 75px;margin-bottom:0;"  />
                   <input type="text" class="kel" name="name" value="" style="width: 36px;margin-bottom:0;"> /
                   <span class="kel_label"><input type="text" class="kel"  value="" style="width: 31px;margin-bottom:0;"></span>
                   <i id="iconplus" class="icon icon-plus input_plus"  style=" visibility: visible;"></i>
                  </div>
                </div>
			 </td>
			</tr>
			<tr>
		        <td style="text-align:left;vertical-align: top;">备注：</td>
				<td><textarea  name="remark" id="remark"  maxlength="32" style="width: 206px;height: 118px;"  title="备注"/></textarea ></td>
			</tr>
			<tr>
		...
		$(function() {
			...
			$("#nutrSelect").change(function(){
				...
				$.ajax({
					...
					//$(this).parent().find("#unit").val(pname)
						//window.document.getElementById("_DialogFrame_0").contentDocument.getElementById("unit").value=pname;
					//	console.log($(this).parent().find("#unit"));
				     ...
				     $.ajax({
						 ...
						 success: function(data){
							 ...
							 for (var i = 0; i < myObject.length; i++) {
									a += "<option value=\'"+myObject[i].id+"\'>"
											+ myObject[i].name + "</option>";
								    //b +="<li id='bigCategoryId_chzn_o_"+(i+1)+"' class='active-result' style=''>"+myObject[i].name+"</li>";
							 }
							 window.parent.document.getElementById("_DialogFrame_0").contentDocument.getElementById("bigCategoryId").innerHTML=a;
							 //$(window.parent.document.getElementById("_DialogFrame_0").contentDocument).find('.chzn-results').eq(0).html('');
							 $chosen.trigger("liszt:updated");
							// console.log($(window.parent.document.getElementById("_DialogFrame_0").contentDocument).find('.chzn-results'));
