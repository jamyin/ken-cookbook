...
<script type="text/javascript">
	...
	function initMaterialDialog() {
		...
		dialog.Drag = true;
		dialog.Title = "原料面板";
		dialog.URL = '<%=basePath%>productController/materialAddPage.do?productId=${productDto.id}&pageInfo='+pageInfo;
		dialog.Width = 485;
		dialog.Height = 255;
	...
	function save(){
	  ...
	  //每份定价校验
	    ...
	    $.ajax({
            cache: true,
            type: "POST",
            url:'<%=basePath%>productController/edit.do',
	...
	<form  name="productForm" id="productForm" method="post">
		...
		<table>
		    <tr>
				...
				<td style="padding-top: 15px;"><input type="text" name="name" id="name"  value="${productDto.name}" maxlength="32"  title="名称" style="width: 136px;"/></td>
				<th></th>
				<td>	<input type="hidden" name="id" id="productId" value="${productDto.id}" /></td>
			...
			<tr >
				...
				<c:forEach items="${listCategory}" var="category">
					<option value="${category.id}" <c:if test="${category.id == productDto.productCategoryId }">selected</c:if>>${category.name}</option>
				</c:forEach>
				</select>
		        <!--添加类别按钮 -->
				...
				<c:forEach items="${listStyle}" var="style">
					<option value="${style.id}" <c:if test="${style.id == productDto.productStyleId }">selected</c:if>>${style.name}</option>
				</c:forEach>
				</select>
				<!--添加菜系按钮 -->
		   ...
		   <tr >
				...
				<c:forEach items="${listTaste}" var="taste">
					<option value="${taste.id}" <c:if test="${taste.id == productDto.productTasteId}">selected</c:if>>${taste.name}</option>
				</c:forEach>
				</select>
				<!--添加口味按钮 -->
				...
				<c:forEach items="${listShape}" var="shape">
					<option value="${shape.id}" <c:if test="${shape.id == productDto.productShapeId}">selected</c:if> >${shape.name}</option>
				</c:forEach>
				</select>
				<!--添加形状按钮 -->
				...
				<c:forEach items="${listColor}" var="color">
					<option value="${color.id}" <c:if test="${color.id == productDto.productColorId}">selected</c:if>>${color.name}</option>
				</c:forEach>
				</select>
				<!--添加颜色按钮 -->
				...
				<c:forEach items="${listCuisine}" var="cuisine">
					<option value="${cuisine.id}" <c:if test="${cuisine.id == productDto.productCuisineId}">selected</c:if>>${cuisine.name}</option>
				</c:forEach>
				</select>
					<!--添加烹饪按钮 -->
			   ...
			   <tr>
				...
				<input type="text" name="materialIdList"   id="materialIdList"   value="${materialIdList}"   maxlength="32" style="display: none;">
				<input type="text" name="materialNameList" id="materialNameList" value="${materialNameList}" maxlength="32" style="width: 136px;" placeholder=""  value="" title="${materialNameList}" readonly="readonly"/>
			</span>
				<!--添加原料按钮 -->
		<!-- 		<i  id="nav-search-icon" class="icon-plus" title="添加原料" ></i> -->
				<!--删除原料按钮 -->
			<!-- 	<i class="icon-remove" id="deleteMaterial" title="删除原料"></i> -->
			</td>
			</tr>
		    <tr>
		        <th style="text-align:left;padding-left: 20px;">营养：</th>
				<td ><input type="text" name="nutrition" id="nutrition"  value="${nutritionList}" maxlength="32" style="width: 186px;"  title="${productDto.nutritionNameList}" readonly="readonly"/></td>
			</tr>
		    <tr>
		        <th style="text-align:left;padding-left: 20px;">单份成本：</th>
				<td >
				<input type="hidden" name="materialWeightList" id="materialWeightList"/>
				<input type="text" name="singleCost" id="singleCost"  value="${productDto.singleCost}" maxlength="32" style="width: 136px;" placeholder="" title="单份成本" readonly="readonly"/><font style="font-weight: bolder;">/元</font></td>
			</tr>
		   <tr>
		        <th style="text-align:left;padding-left: 20px;">每份定价：</th>
				<td ><input type="text" name="eachPricing" id="eachPricing" value="${productDto.eachPricing}"  maxlength="32" style="width: 136px;" placeholder="输入每份定价" title="每份定价"/><font style="font-weight: bolder;">/元</font></td>
			</tr>
		   <tr>
		        <th style="text-align:left;padding-left: 20px;">单份克重：</th>
				<td><input type="text" name="singleWeight" id="singleWeight"  value="${productDto.singleWeight}" maxlength="32" style="width: 136px;" placeholder="输入单份克重" title="单份克重"/><font style="font-weight: bolder;">/克</font></td>
			</tr>
			   <tr>
		        <th style="text-align:left;padding-left: 20px;">需求数：</th>
				<td ><input type="text" name="demandNumber" id="demandNumber" value="${productDto.demandNumber}" maxlength="32" style="width: 136px;" placeholder="输入需求数" title="需求数"/><font style="font-weight: bolder;">/份</font></td>
			</tr>
			   <tr>
		        <th style="text-align:left;padding-left: 20px;">总成本：</th>
				<td ><input type="text" name="totalCost" id="totalCost"  value="${productDto.totalCost}" maxlength="32" style="width: 136px;" title="总成本" readonly="readonly"/><font style="font-weight: bolder;">/元</font></td>
			</tr>
			<tr>
		        <th style="text-align:left;padding-left: 20px;">备注：</th>
				<td><input type="text" name="remark" id="remark"  value="${productDto.remark}" maxlength="32" style="width: 190px;height: 48px;"  title="备注"/></td>
			</tr>
			<tr>
				     ...
				     $.ajax({
						 ...
						 success: function(data){
							 ...
							 for (var i = 0; i < myObject.length; i++) {
							  ...
							  //获取编辑框的烹饪元素
							 top.Dialog.getDialogById("product_edit").getElementById("productCuisineId").innerHTML=a;
							 var product_cuisineId="${productDto.productCuisineId}";
					         //先给修改后值得下拉框赋值,然后选中之前的值;最后再刷新下拉框
						     top.Dialog.getDialogById("product_edit").setSelectedValue("#productCuisineId",product_cuisineId);
				     ...
				     $.ajax({
						 ...
						 success: function(data){
							 ...
							 for (var i = 0; i < myObject.length; i++) {
							  ...
							  //获取编辑框的颜色元素
							 top.Dialog.getDialogById("product_edit").getElementById("productColorId").innerHTML=a;
							 var product_colorId="${productDto.productColorId}";
					         //先给修改后值得下拉框赋值,然后选中之前的值;最后再刷新下拉框
						     top.Dialog.getDialogById("product_edit").setSelectedValue("#productColorId",product_colorId);
							 $chosen.trigger("liszt:updated");
					      }
				     });
				     colorDialog.close();
				     ...
				     $.ajax({
						 ...
						 success: function(data){
							 ...
							 for (var i = 0; i < myObject.length; i++) {
							  ...
							  //获取编辑框的形状元素
							 top.Dialog.getDialogById("product_edit").getElementById("productShapeId").innerHTML=a;
							 var product_shapeId="${productDto.productShapeId}";
					         //先给修改后值得下拉框赋值,然后选中之前的值;最后再刷新下拉框
						     top.Dialog.getDialogById("product_edit").setSelectedValue("#productShapeId",product_shapeId);
							 $chosen.trigger("liszt:updated");
					      }
				     });
				     styleDialog.close();
				     ...
				     $.ajax({
						 ...
						 success: function(data){
							 ...
							 for (var i = 0; i < myObject.length; i++) {
							  ...
							  //获取编辑框的口味元素
							 top.Dialog.getDialogById("product_edit").getElementById("productTasteId").innerHTML=a;
							  
							 var product_tasteId="${productDto.productTasteId}";
					         //先给修改后值得下拉框赋值,然后选中之前的值;最后再刷新下拉框
						     top.Dialog.getDialogById("product_edit").setSelectedValue("#productTasteId",product_tasteId);
							 $chosen.trigger("liszt:updated");
					      }
				     });
				     styleDialog.close();
				     ...
				     $.ajax({
						 ...
						 success: function(data){
							 ...
							 for (var i = 0; i < myObject.length; i++) {
							  ...
							  //获取编辑框的菜系元素
							 top.Dialog.getDialogById("product_edit").getElementById("productStyleId").innerHTML=a;
							  
							 var product_styleId="${productDto.productStyleId}";
					         //先给修改后值得下拉框赋值,然后选中之前的值;最后再刷新下拉框
						     top.Dialog.getDialogById("product_edit").setSelectedValue("#productStyleId",product_styleId);
							 $chosen.trigger("liszt:updated");
					      }
				     });
				     styleDialog.close();
				 ...
				 categoryDialog.CancelEvent = function(){ //关闭事件
				     ...
				     $.ajax({
						 ...
						 success: function(data){
						    ...
						    //获取编辑框的类别元素
							 top.Dialog.getDialogById("product_edit").getElementById("productCategoryId").innerHTML=a;
							 //window.parent.document.getElementById("_DialogFrame_1").contentDocument.getElementById("productCategoryId").innerHTML=a;
		
						     var category_Id="${productDto.productCategoryId}";
					         //先给修改后值得下拉框赋值,然后选中之前的值;最后再刷新下拉框
						    // $("#productCategoryId option[value='"+category_Id+"']").prop("selected","selected");
					         top.Dialog.getDialogById("product_edit").setSelectedValue("#productCategoryId",category_Id);
						     $chosen.trigger("liszt:updated");
						   }
				     });
